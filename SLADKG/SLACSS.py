...
def _env_int(name: str, default: int) -> int:
    ...
    if raw is None or raw == "":
        return default
    try:
        return int(raw, 0)  # supports decimal/hex literals
    except ValueError as exc:
        raise ValueError(f"Invalid integer for {name}: {raw!r}") from exc


# Ring parameters (override via env vars for quick experiments).
# - SLACSS_Q: modulus q (must be a prime for Shamir-style interpolation in this demo)
# - SLACSS_RING_DEGREE: ring dimension n in Z_q[X]/(X^n+1)
PRIME = _env_int("SLACSS_Q", 12289)
RING_DEGREE = _env_int("SLACSS_RING_DEGREE", 8)  # degree n of X^n + 1
